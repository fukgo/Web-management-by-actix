...
pub async fn post_product_type(pool: web::Data<r2d2::Pool<ConnectionManager<MysqlConnection>>>,new_product_type:web::Json<ProductTypesCreate>)->Result<HttpResponse, EveryError>{
    let mut one_pool = pool.get().expect("couldn't get db connection from pool");
    let product_type_detail = post_product_type_sql(&mut one_pool, new_product_type.into_inner()).await?;
    Ok(HttpResponse::Ok().json(Response{status:200,data:product_type_detail}))

}
pub async fn post_product(pool: web::Data<r2d2::Pool<ConnectionManager<MysqlConnection>>>,new_product:web::Json<ProductCreate>)->Result<HttpResponse, EveryError>{
    let mut one_pool = pool.get().expect("couldn't get db connection from pool");
